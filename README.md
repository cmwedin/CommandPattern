...
Another more advanced type of command you can use is [CompositeCommands](Runtime/Commands/CompositeCommand.cs). This abstract subclass of Command encapsulates multiple commands into a single object. When its `Execute()` method is invoked it invokes the `Execute()` methods of all the objects it encapsulates in turn. Further a [SimpleComposite](Runtime/Commands/SimpleComposite.cs) is a concrete type of CompositeCommand's that can be created from any collection of Commands which does not contain a Command that is IFailable.
If the reasoning for this restriction is not obvious any explanation can be insightful into the concepts behind the command pattern.
